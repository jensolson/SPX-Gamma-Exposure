...
def CBOE_Greeks(filename, low, high, incr, expiry, field):
    ...
    pltTime = raw.loc[0][0][-9:-1]
    dtDate = datetime.datetime.strptime(pltDate, '%b %d %Y').date()

    # Extract dataframe for analysis
    raw = pd.read_table(filename, sep=',', header=2)
    c = raw.loc[:, :'Open Int'].copy(deep=True)
    c.columns = c.columns.str.replace('Calls', 'ID')
    p = raw.loc[:, 'Puts':'Open Int.1'].copy(deep=True)
    p.columns = p.columns.str.replace('Puts', 'ID')
    p.columns = p.columns.str.replace('.1', '')
    ...
    df['Expiry'] = pd.to_datetime(df['ID'].str[:11], infer_datetime_format=True)
    ...
    #df['DTE'] = (df['Expiry']-pd.to_datetime(datetime.date.today(), infer_datetime_format=True)).dt.days
    #df['DTE'] = (df['Expiry']-pd.to_datetime(dtDate, infer_datetime_format=True)).dt.days

    df = df.loc[#(df['Last Sale'] >= 0.10) &\
                (df['Open Int'] > 10) &\
                #(df['Bid'].notnull() & df['Ask'].notnull()) &\
                (df['Bid'] > .05) &\
                (df['BDTE'] >= 1) #&\
                #(df['DTE'] < 365*1)
               ].copy(deep=True)
    ...
    df['Mid'] = np.mean(df[['Bid', 'Ask']], axis=1)
    df['Strike'] = pd.to_numeric(df['ID'].str[12:19], errors='coerce', downcast='signed')
